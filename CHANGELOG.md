...
- Model artifact downloads now use the direct Hugging Face `resolve` URL via `urllib.request`, matching the registry fetch and the GitHub Actions workflow. The `huggingface_hub` package is no longer required to download model weights (it remains an optional dependency for catalog resolution).
